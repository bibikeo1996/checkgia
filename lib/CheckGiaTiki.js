...
function CheckGiaTiki(req,res){
    ...
    for (var doithu of urls) {
        try {
            const link = doithu.Name.toLowerCase();//.replace(/[^A-Z0-9]+/ig, "")
            await page.goto("https://tiki.vn/search?q="+link+"&sort=price%2Casc&page=1", {waitUntil: 'networkidle2', timeout: 100000});
            const info = await page.evaluate(() => {
                let checkweb = document.querySelector("div.product-listing");
                if (checkweb !== null) {
                    const checkname = document.querySelector("ul.breadcrumb li span");
                    const name = document.querySelector(checkname !== null ? "ul.breadcrumb li span" : ".khongcoclass")
                   ...
                   function CheckGianHang(){
                    const checkname = document.querySelector("ul.breadcrumb li span").innerText.toLowerCase();
                    const done = [];
                    // lấy tất cả gian hàng theo keyword
                    const laygianhang = document.querySelectorAll(".product-item");
                    for(var i = 0; i < laygianhang.length; i++){
                        //lấy tên sản phẩm theo từng gian hàng
                        const getdata = laygianhang[i].querySelector("a.search-a-product-item").getAttribute("title");
                        const data = getdata.toLowerCase();
                        //so sánh tên sản phẩm của từng gian hàng với keyword
                            ...
                            const gia = laygianhang[i].querySelector("span.final-price").innerText;
                            done.push(gia);                          
                            }else{
                                done.push("Error!");
                            }
